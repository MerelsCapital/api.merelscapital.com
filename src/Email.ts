...
export class Email {
    ...
    public async sendInternalBookingConfirmation(toEmail: string, clientName: string, time: Temporal.ZonedDateTime, meetingUrl: URL, details: string, iCalString?: string): Promise<Result<boolean, Error>> {
        const start = Temporal.ZonedDateTime.from(time);
        ...
        try {
            await this.resend.emails.send({
                ...
                html: `
                    <h2>New booking with ${clientName} (${toEmail}),</h2>
                    <p><strong>Date:</strong> ${start.toLocaleString('en-US', { dateStyle: 'long', timeStyle: 'short' })}</p>
                    <p><strong>Join via this meeting link:</strong> <a href="${meetingUrl}">${meetingUrl}</a></p>
